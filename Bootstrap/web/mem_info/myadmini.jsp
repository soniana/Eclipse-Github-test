...
<!-- 	 <link rel="stylesheet" href="css/meminfo_check.css"> -->
	...
	function bclick(f){
		f.method='POST';
		f.action='mem_info.show?db=search';
		f.submit();
	}
	function cclick(f){
		f.method='POST';
		f.action='cominfo.show?db=search';
		f.submit();
	}
	</script>
						...
						<div class="row">
							<div class="col-xs-6" id="form-l">
								<form id="loginForm" method="POST" action="login.show?db=mem" novalidate="novalidate">
									...
									<input type="button" id="button" class="button-style wow fadeInDown" value="일반 회원 정보 확인" onclick="bclick(this.form);">
								</form>
							</div>
							<div class="col-xs-6" id="form-r" >
								<form id="loginForm" method="POST" action="login.show?db=mem" novalidate="novalidate">
									<img src="./images/browser.png"> <br> <br>
									<input type="button" id="button" class="button-style wow fadeInDown" value="사업자 회원 정보 확인" onclick="cclick(this.form);">
								</form>
							</div>
						</div>
					</div>
