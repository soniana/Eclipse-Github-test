...
function bclick(f){
	f.method='POST';
	f.action='cominfo.show?db=detail&comid=${logincom.comid}';
	f.submit();
}
function cclick(f){
	f.method='POST';
	f.action='cominfo.show?db=mylist&comid=${logincom.comid}';
	f.submit();
}
</script>
						...
						<div class="row">
							...
							<div class="col-xs-6" id="form-r" >
								<form id="loginForm" method="POST" action="login.show?db=mem" novalidate="novalidate">
									...
									<input type="button" id="button" class="button-style wow fadeInDown" value="내 공연 목록" onclick="cclick(this.form);">
								</form>
							</div>
						</div>
					</div>
